...
class NewsParser:
    ...
    def pretty_text(self, index_content_list):
        contents = list()
        for each in index_content_list:
            sub_text = self.substring(each[1][0])
            if not sub_text:
                continue
            else:
    ...
    def extract_news(self, html):
        ...
        html = self.line_div(html)
        cluster_para, absorb_para = self.extract_paragraph(html)
        if cluster_para:
            index, feature = self.extract_feature(cluster_para)
            skeleton_dict = self.gen_skeleton(cluster_para, index, feature)
            if skeleton_dict:
                if absorb_para:
                    content_dict = self.absorb_text(skeleton_dict, absorb_para)
                else:
                    content_dict = skeleton_dict
        ...
        def extract_content():
            ...
            if index_content_list:
                text = self.pretty_text(index_content_list)
                text = text.strip()
            return text
